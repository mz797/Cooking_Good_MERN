...
export const getAllCategories = (
  ...
  Category.find()
    ...
    .catch((err) => {
      ...
      const error = new HttpError("Nie udało się pobrać listy kategorii", 500);
      return next(error);
    });
};
...
export const getOneCategory = (
  ...
  Category.findById(id)
    ...
    .then((category) => {
      if (!category) {
        throw new HttpError("Nie znaleziono kategorii", 404);
      }
...
export const deleteCategory = async (
  ...
  try {
    ...
    console.log(err);
    const error = new HttpError("Błąd wyszukiwania categorii w bazie.", 500);
    return next(error);
  }
