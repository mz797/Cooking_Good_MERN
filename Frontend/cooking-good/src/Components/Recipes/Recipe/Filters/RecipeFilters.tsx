...
import FridgeDialog from "./FridgeDialog";
import ForbidenIngredients from "./ForbidenIngredients";
...
const RecipeFilters = ({ categoriesList, recipeList, onFilter }: myProps) => {
  ...
  const [fridgeOpen, setFridgeOpen] = useState<boolean>(false);
  const [fridgeValue, setFridgeValue] = useState<string>("");

  const [forbidenIngOpen, setForbidenIngOpen] = useState<boolean>(false);
  const [forbidenIngValue, setForbidenIngValue] = useState<string>("");
      ...
      preparedList = preparedList.filter((recipe) =>
        recipe.ingredients.some((ingredient) =>
          fridgeValue
            .trim()
            .toLowerCase()
            .includes(ingredient.name.trim().toLowerCase())
        )
      );
    }
    if (!!forbidenIngValue) {
  ...
  return (
    <>
      ...
      />
      <ForbidenIngredients
        defaultValue={forbidenIngValue}
        open={forbidenIngOpen}
        onClose={() => setForbidenIngOpen(false)}
        onSave={(value: string) => {
          setForbidenIngValue(value);
          setForbidenIngOpen(false);
        }}
      />
      <Stack direction="row" spacing={1} sx={{ my: 2 }}>
        ...
        <Badge badgeContent={!!fridgeValue ? "!" : null} color="primary">
          <Button variant="outlined" onClick={() => setFridgeOpen(true)}>
            <KitchenIcon />
          </Button>
        </Badge>
        <Badge badgeContent={!!forbidenIngValue ? "!" : null} color="primary">
          <Button variant="outlined" onClick={() => setForbidenIngOpen(true)}>
            <NoMealsIcon />
          </Button>
        </Badge>
